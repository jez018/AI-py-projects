...
clf3.fit(new_df[['A','B','C','D']].iloc[0:new_df.A.count()-3], new_df.target_df.iloc[0:new_df.A.count()-3])

#Prediction part
#to_predict = np.array([[19464.531705, 18813.124760, 19045.020273, 19113.933395]])
to_predict = np.array([[19749.263672, 19845.994141, 19308.337891, 19566.177734]])
#far_pred = np.array([[223224.45414, 23623.88553, 24581.00617, 26381.29623]])
#far_act = 26389.29026
to_predict = DataFrame(to_predict, columns=['A','B','C','D'])
print('actual: ', 19511.396484)
...
print('Diffrence', 19511.396484-clf3.predict(to_predict))
